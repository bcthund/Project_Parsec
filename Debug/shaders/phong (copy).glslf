...
in Data {
 ...
 smooth in vec3 vRayDir[32];
 //smooth in vec3 vLightPos[32];
} data;
...
void main(void) {
	...
	if (uiType[0] == uint(1)) {
		...
		fMaxDarkness = 0.1;
		float fDist = length(data.vLight);
		float fAtt = vAttenuation[0].x + (vAttenuation[0].y * fDist) + (vAttenuation[0].z * fDist * fDist);
		float nxLight = min(max(fMaxDarkness, dot(data.vNormal.xyz, normalize(data.vLight))),1.0);
