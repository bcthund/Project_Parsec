...
in vec2 texCoords;
uniform float resolution = 1280.0f/2;	// divide because of the smaller image size rendered by FBO
...
void main(void) {
 ...
 //vFragColor += texture(colorMap, blurTexCoords[10])* fFactor[10];
    ...
    for (int i=1; i<5; i++) {
        vFragColor += texture2D(colorMap, (texCoords + (vec2(offset[i])/resolution, 0.0)) ) * weight[i];
        vFragColor += texture2D(colorMap, (texCoords - (vec2(offset[i])/resolution, 0.0)) ) * weight[i];
    }
}
